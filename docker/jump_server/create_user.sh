...
USER_GROUP="captain_users"

useradd -m -d /home/$USERNAME -s /bin/bash -g $USER_GROUP $USERNAME
...
if grep -q "$PUBLIC_KEY" $USER_SSH_DIR/authorized_keys; then
    ...
    echo "\n$PUBLIC_KEY" >> $USER_SSH_DIR/authorized_keys
fi
chmod 700 $USER_SSH_DIR
chmod 600 $USER_SSH_DIR/authorized_keys
chown -R $USERNAME:$USER_GROUP $USER_SSH_DIR
